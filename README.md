...
| `WRITE_PERSONAL_EMAIL_TO_CONTACTS` | Куда записывать личный email из CSV: в структуру Contacts (`true`, регистронезависимо) или в поле About (`false`) | Нет (по умолчанию `false` — запись в About) | `true` |

### Параметр WRITE_PERSONAL_EMAIL_TO_CONTACTS

Параметр управляет тем, куда записывается значение поля `personal_email` из CSV-файла:

- **`WRITE_PERSONAL_EMAIL_TO_CONTACTS=false`** (или параметр отсутствует, любое другое значение) — личный email записывается в поле **About** пользователя в виде JSON-строки:
  ```json
  {"personal_email": "alexander.ivanov@example.com"}
  ```

- **`WRITE_PERSONAL_EMAIL_TO_CONTACTS=true`** (регистронезависимо: `TRUE`, `True` и т.д.) — личный email записывается в структуру **Contacts** пользователя:
  ```json
  {"type": "email", "value": "alexander.ivanov@example.com", "label": "Personal"}
  ```
  В контактах пользователя поддерживается только один контакт с label `Personal`.

**Особенности поведения при обновлении пользователей:**
- Если контакт email/Personal уже существует и в CSV передано новое значение — оно перезаписывается
- Если в CSV передан символ очистки `-` — контакт удаляется (в режиме About — удаляется ключ из JSON в `about`)
- Если поле `personal_email` в CSV пустое — существующее значение не изменяется
- **Миграция данных между режимами не выполняется**: при переключении параметра старые значения остаются на прежнем месте, новые записи идут только в выбранное место

> [!NOTE]
> Письма-уведомления (приветственное и о смене пароля) отправляются на `personal_email` независимо от выбранного режима. При смене пароля email пользователя ищется сначала в CSV, затем — в зависимости от параметра — в структуре Contacts или в поле About.
...
# Куда записывать личный email из CSV:
# false (по умолчанию) - в поле About пользователя
# true - в структуру Contacts ({"type": "email", "value": "...", "label": "Personal"})
WRITE_PERSONAL_EMAIL_TO_CONTACTS=false
...
> [!NOTE]
> - Личный email адрес по умолчанию записывается в виде JSON строки в атрибут `about` пользователя (см. параметр `WRITE_PERSONAL_EMAIL_TO_CONTACTS`). При `WRITE_PERSONAL_EMAIL_TO_CONTACTS=true` он записывается в структуру Contacts с label `Personal`. Предполагается его использовать для уведомлений по почте пользователя о различных действиях с его учёткой в Я360 (в том числе, о новом пароле в случае сброса администратором)
...
4. **Email-уведомления**:
   ...
   - Email берется из поля `personal_email` CSV-файла, а если он не указан — в зависимости от параметра `WRITE_PERSONAL_EMAIL_TO_CONTACTS`: из структуры Contacts (label `Personal`) или из поля `about` пользователя (где хранится в формате JSON)
   - Если `personal_email` указан в CSV, он обновляется в соответствии с параметром `WRITE_PERSONAL_EMAIL_TO_CONTACTS`
...
| `personal_email` | Если указан → обновляется в поле `about` (JSON) или в структуре Contacts (email с label `Personal`) в зависимости от параметра `WRITE_PERSONAL_EMAIL_TO_CONTACTS`. Символ очистки `-` удаляет значение. Пустое поле → не изменяется. |
...
- `personal_email` берется из CSV-файла, а если он не указан — из структуры Contacts (при `WRITE_PERSONAL_EMAIL_TO_CONTACTS=true`) или из поля `about` пользователя
...
- Personal email хранится в поле `about` (JSON) или в структуре Contacts в зависимости от параметра `WRITE_PERSONAL_EMAIL_TO_CONTACTS`
